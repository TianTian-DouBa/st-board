    ...
    def valid_utd(self):
        ...
        if isinstance(self.df_idt, pd.DataFrame):
            ...
            if idt_date == source_date:
                ...
                add_log(40, '[fn]Indicator.valid_utd() ts_code:{0[0]} {0[1]} not uptodate. idt:{0[2]} source:{0[3]}',
                        log_args)
        else:
...
class Ma(Indicator):
    ...
    def __new__(cls, ts_code, par_asset, idt_type, idt_name, period, source='close_hfq', reload=False, update_csv=True,
                subtype='D'):
        """
    ...
    def __init__(self, ts_code, par_asset, idt_type, idt_name, period, source='close_hfq', reload=False,
                 update_csv=True, subtype='D'):
        """
        period:<int> 周期数
        subtype:<str> 'D'-Day; 'W'-Week; 'M'-Month #only 'D' yet
        """
        Indicator.__init__(self, ts_code=ts_code, par_asset=par_asset, idt_type=idt_type, reload=reload,
    ...
    def _calc_res(self):
        ...
        if isinstance(df_idt, pd.DataFrame):
            ...
            if idt_head_in_source == 0:
                log_args = [self.ts_code]
                add_log(40, '[fn]Ma._calc_res() ts_code:{0[0]}; idt_head up to source date, no need to update',
                        log_args)
                return
...
class Ema(Indicator):
    ...
    def _calc_res(self):
        ...
        if isinstance(df_idt, pd.DataFrame):
            ...
            if idt_head_in_source == 0:
                log_args = [self.ts_code]
                add_log(40, '[fn]Ema._calc_res() ts_code:{0[0]}; idt_head up to source date, no need to update',
                        log_args)
                return
            elif idt_head_in_source > 0:
...
class Macd(Indicator):
    ...
    def __new__(cls, ts_code, par_asset, idt_type, idt_name, long_n1=26, short_n2=12, dea_n3=9, source='close_hfq',
                reload=False, update_csv=True, subtype='D'):
        """
        source:<str> e.g. 'close_hfq' #SOURCE
    ...
    def __init__(self, ts_code, par_asset, idt_type, idt_name, long_n1=26, short_n2=12, dea_n3=9, source='close_hfq',
                 reload=False, update_csv=True, subtype='D'):
        """
        subtype:<str> 'D'-Day; 'W'-Week; 'M'-Month #only 'D' yet
        """
    ...
    def _calc_res(self):
        ...
        if isinstance(df_idt, pd.DataFrame):
            ...
            if idt_head_in_source == 0:
                log_args = [self.ts_code]
                add_log(40, '[fn]Macd._calc_res() ts_code:{0[0]}; idt_head up to source date, no need to update',
                        log_args)
                return
            elif idt_head_in_source > 0:
